...
def correct_state(uri)
    type, subtype, state = uri.match(%r{/([A-z0-9-]*)-near-me(?:/(indoor|outdoor|covered))?/([A-z| |\-|%20]*)(/[A-z| |\-|%20]*)*$}i).captures
    state.gsub!(/%20| /, '-')

    "#{type.downcase}-near-me#{"/#{subtype.downcase}" unless subtype.nil?}/#{updated_state(state.downcase)}"
end
